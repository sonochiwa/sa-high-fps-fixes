...
| `bikePitchExperiment` | `1` | Narrow trial for the excessive backward pitch acquired at takeoff. It touches the wheel-contact `ApplyTurnForce` at `0x6D7B17` only after both front suspension lines have cleared the ramp while the bike is moving upward faster than `0.02`; this includes the rear-wheel takeoff phase and GTA's short stale-contact tail. Level-ground wheelies, nose-down pitch, yaw, roll, and 30 FPS or below remain stock. |
