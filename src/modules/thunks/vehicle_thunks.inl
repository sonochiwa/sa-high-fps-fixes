...
// Both stand in for the `cmp`/`je` that guards the wheel's skid classification.
// The FIXED test is answered first and unchanged, so a wheel the brake already
// pinned keeps its stock path. Otherwise a slip that would not have saturated
// the budget at 30 FPS jumps into the game's own `tractionLoss = 1.0` arm,
// which skips the state assignment and falls into the clamp untouched. Flags are
// dead here -- the game's own first instruction is a `cmp` -- but they are saved
// anyway, and the answer travels in a global because `popfd` and `pop eax` would
// otherwise destroy it. The FPU stack is not touched: it holds one value here,
// and every exit expects exactly that.
__declspec(naked) void CarSkidStateThunk() {
    __asm {
        pushfd
        push eax
        push ecx
        push edx
        push dword ptr [esp + 0x88]
        push dword ptr [esp + 0x9C]
        call MarkCarSkidState
        add esp, 8
        pop edx
        pop ecx
        pop eax
        ...
        cmp dword ptr [ebp], 3
        je stateDone
        cmp byte ptr [g_skipSkidState], 0
        jne skipState
        jmp kCarSkidStateReturn
    skipState:
        jmp kCarSkidTractionOne
    stateDone:
        jmp kCarSkidStateDone
    }
}

__declspec(naked) void BikeSkidStateThunk() {
    __asm {
        pushfd
        push eax
        push ecx
        ...
        cmp byte ptr [g_skipSkidState], 0
        jne skipState
        jmp kBikeSkidStateReturn
    skipState:
        jmp kBikeSkidTractionOne
    stateDone:
        jmp kBikeSkidStateDone
    }
}
