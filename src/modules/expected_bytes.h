...
// fmul [esp+18h] / fmul [0x871058], and the same pair with the operands the
// other way round at the second car site.
constexpr std::array<uint8_t, 10> kExpectedCarSteerInputA{
    0xD8, 0x4C, 0x24, 0x18, 0xD8, 0x0D, 0x58, 0x10, 0x87, 0x00
};
constexpr std::array<uint8_t, 10> kExpectedCarSteerInputB{
    0xD8, 0x0D, 0x58, 0x10, 0x87, 0x00, 0xD8, 0x4C, 0x24, 0x18
};
constexpr std::array<uint8_t, 10> kExpectedBikeSteerInput{
    0xD8, 0x4C, 0x24, 0x18, 0xD8, 0x0D, 0x00, 0x13, 0x87, 0x00
};
// fst [esp+18h] / jne +79h: the lateral slip stored, then the driving test
constexpr std::array<uint8_t, 6> kExpectedWheelSlipRight{
    0xD9, 0x54, 0x24, 0x18, 0x75, 0x79
};
// fchs / fstp [esp+10h]: the coasting longitudinal slip stored
constexpr std::array<uint8_t, 6> kExpectedWheelSlipCoast{
    0xD9, 0xE0, 0xD9, 0x5C, 0x24, 0x10
};
// push esi / mov esi,ecx / mov al,[esi+36h] / mov cl,al: the entry of
// CPhysical::ProcessControl
constexpr std::array<uint8_t, 8> kExpectedPhysicalProcessControl{
    0x56, 0x8B, 0xF1, 0x8A, 0x46, 0x36, 0x8A, 0xC8
};
// sub esp,28h / mov eax,[esp+3Ch]: the entry of CPhysical::ApplySpringDampening
constexpr std::array<uint8_t, 7> kExpectedApplySpringDampening{
    0x83, 0xEC, 0x28, 0x8B, 0x44, 0x24, 0x3C
};
// fld st(0) / fsub [ebp] / jne +8
constexpr std::array<uint8_t, 7> kExpectedTransmissionInertia{
    0xD9, 0xC0, 0xD8, 0x65, 0x00, 0x75, 0x08
};
// fld 1.0 / mov eax,[esp+28h] / fsub [0.85] / fmulp / fld [0.85] / fmul [eax] / faddp
constexpr std::array<uint8_t, 28> kExpectedTransmissionSmoother{
    0xD9, 0x05, 0x24, 0x86, 0x85, 0x00, 0x8B, 0x44, 0x24, 0x28,
    0xD8, 0x25, 0x7C, 0x34, 0x8D, 0x00, 0xDE, 0xC9,
    0xD9, 0x05, 0x7C, 0x34, 0x8D, 0x00, 0xD8, 0x08, 0xDE, 0xC1
};
