...
// Both replace the `fld`/`fmul` pair that squared `adhesion` for the wheel's
// saturation test, and both leave exactly one value on the FPU stack, the
// squared threshold, which is what that pair left. The adhesion slot is read
// from the frame the game was using, offset past the four dwords pushed here.
__declspec(naked) void CarSkidThresholdThunk() {
    __asm {
        pushfd
        push eax
        push ecx
        ...
        call GetCarSkidThresholdSquared
        add esp, 4
        pop edx
        pop ecx
        pop eax
        popfd
        jmp kCarSkidThresholdReturn
    }
}

__declspec(naked) void BikeSkidThresholdThunk() {
    __asm {
        pushfd
        push eax
        push ecx
        ...
        call GetBikeSkidThresholdSquared
        add esp, 4
        pop edx
        pop ecx
        pop eax
        popfd
        jmp kBikeSkidThresholdReturn
    }
}
